...
const workflowSteps = [
  {
    ...
    desc: 'We kick off with deep strategic discussions to align on your targets, core users, challenges, and tech specifications.',
    icon: MessageSquare,
    accent: '#FF8706',
    bgColor: '#FFEFE5'
  },
  {
    ...
    desc: 'Our senior architects construct robust specifications, outline system dependencies, and map out milestones.',
    icon: FileText,
    accent: '#14B8B0',
    bgColor: '#E6F8F9'
  },
  {
    ...
    desc: 'We forge beautiful, pixel-perfect user journeys and interactive prototypes designed for absolute usability.',
    icon: Palette,
    accent: '#7C3AED',
    bgColor: '#F0ECFF'
  },
  {
    ...
    desc: 'Our engineers build clean, highly optimized codebases with continuous testing and robust integrations.',
    icon: Code2,
    accent: '#00C2CC',
    bgColor: '#E3FAF6'
  },
  {
    step: '05',
    title: 'Launch & Expansion Support',
    desc: 'We launch securely to production, monitor parameters, and offer continuous updates to ensure business growth.',
    icon: Rocket,
    accent: '#FF6B00',
    bgColor: '#FFF0E5'
  }
...
export const ServicesPage: React.FC<{ onContactClick: () => void }> = ({ onContactClick }) => {
  ...
  useEffect(() => {
    if (selectedService || isWorkModalOpen) {
  ...
  }, [selectedService, isWorkModalOpen]);
        ...
        {isWorkModalOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[92%] max-w-3xl max-h-[85vh] bg-white rounded-[32px] border border-slate-100 shadow-2xl z-50 overflow-hidden flex flex-col"
            >
              {/* Close Button */}
              <button
                onClick={() => setIsWorkModalOpen(false)}
                className="absolute top-6 right-6 w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors cursor-pointer select-none group z-10 bg-transparent border-none"
              >
                <X className="w-4 h-4 text-slate-500 group-hover:text-slate-800 transition-colors" />
              </button>

              {/* Header */}
              <div className="p-8 sm:p-10 border-b border-slate-100 relative overflow-hidden bg-slate-50/50 shrink-0">
                <div
                  className="absolute -top-24 -right-24 w-60 h-60 rounded-full blur-3xl opacity-15 pointer-events-none bg-[#FF8706]"
                />

                <div className="flex items-center gap-3.5 mb-2">
                  <div className="w-10 h-10 rounded-xl bg-[#FFEFE5] text-[#FF8706] flex items-center justify-center shrink-0">
                    <Zap className="w-5 h-5 stroke-[2.5]" />
                  </div>
                  <span className="text-xs font-black px-3 py-1 rounded-full bg-[#FF8706]/10 text-[#FF8706] uppercase tracking-wider">
                    Our Workflow
                  </span>
                </div>
                
                <h2 className="font-display font-extrabold text-2xl sm:text-3xl text-[#0D152A] leading-tight">
                  How We Work
                </h2>
                <p className="text-[#6B7280] font-medium text-sm mt-1.5 max-w-xl">
                  A structured, transparent development methodology designed to transform your business goals into reliable digital products.
                </p>
              </div>
                ...
                {workflowSteps.map((step, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="flex gap-4 sm:gap-6 bg-slate-50/40 hover:bg-slate-50/90 rounded-2xl p-5 border border-slate-100 hover:border-slate-200 transition-all duration-300 relative group"
                    >
                      {/* Left side step numbering & icon */}
                      <div className="flex flex-col items-center shrink-0">
                        <div
                          className="w-12 h-12 rounded-2xl flex items-center justify-center shadow-xs"
                          style={{ backgroundColor: step.bgColor }}
                        >
                          <StepIcon className="w-5 h-5 stroke-[2.2]" style={{ color: step.accent }} />
                        </div>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span 
                            className="font-display font-black text-xs px-2 py-0.5 rounded-md"
                            style={{ backgroundColor: `${step.accent}15`, color: step.accent }}
                          >
                            Step {step.step}
                          </span>
                          <h3 className="font-display font-extrabold text-base sm:text-lg text-[#0D152A] tracking-tight group-hover:text-[#FF8706] transition-colors">
                            {step.title}
                          </h3>
                        </div>
                        <p className="text-[#6B7280] font-medium text-xs sm:text-sm leading-relaxed max-w-2xl">
                          {step.desc}
                        </p>
                      </div>
                    </motion.div>
              ...
              {/* Footer CTA */}
              <div className="p-6 sm:p-8 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/20 shrink-0">
                <p className="text-[#667085] font-medium text-xs sm:text-sm text-center sm:text-left">
                  Ready to launch your project with our step-by-step process?
                </p>
                <button
                  onClick={() => {
                  ...
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 bg-[#FF8706] hover:bg-[#E07200] text-white font-bold text-sm px-6 py-3 rounded-2xl shadow-lg shadow-[#FF8706]/20 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
                >
